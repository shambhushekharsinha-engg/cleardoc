...
export default function AnalysisDashboard({
  ...
  documentFile,
  onReset,
  chatState, // { messages, isChatLoading, sendMessage, clearChat }
}) {
  const [copiedAll, setCopiedAll] = useState(false);
  const [activeTab, setActiveTab] = useState('analysis'); // 'analysis' | 'architecture'
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-3xl p-5 sm:p-6 shadow-xl shadow-slate-200/40 border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center bg-slate-100 rounded-full p-1 border border-slate-200">
            <button
              ...
              className={`inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                activeTab === 'analysis' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                activeTab === 'architecture' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
          ...
          <button
            onClick={handlePrint}
            className="hidden sm:inline-flex items-center space-x-1.5 px-4 py-2 rounded-full text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors shadow-2xs"
          ...
          {/* Seamless reset button */}
          <button
            onClick={onReset}
            className="inline-flex items-center space-x-2 px-5 py-2 rounded-full text-xs sm:text-sm font-bold bg-slate-900 hover:bg-indigo-600 text-white transition-colors shadow-md shadow-slate-200"
          >
      ...
      {activeTab === 'architecture' ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            {/* Red Flags & Liabilities */}
            <FlagCard type="red" flags={redFlags} />

            {/* Green Flags & Protections */}
            <FlagCard type="green" flags={greenFlags} />
          </div>

          {/* Right Column: Chat Sidebar (Isolated typing state) */}
